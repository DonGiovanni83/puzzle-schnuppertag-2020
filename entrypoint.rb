...
if ENV['INPUT_BUILD-ONLY'] == "true"
  system_or_fail('bundle', 'exec', 'jekyll', 'build', '--future', '--verbose', '--trace', '--config', '_config.yml')
  exit
else
  system_or_fail('bundle', 'exec', 'jekyll', 'build', '--verbose', '--trace', '--config', '_config.yml')
end
